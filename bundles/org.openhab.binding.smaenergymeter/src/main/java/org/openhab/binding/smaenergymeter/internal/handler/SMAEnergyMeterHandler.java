...
public class SMAEnergyMeterHandler extends BaseThingHandler implements PayloadHandler, Runnable {
    ...
    public void initialize() {
        ...
        try {
            ...
            this.updateInterval = TimeUnit.SECONDS.toMillis(config.getPollingPeriod() == null ? 30 : config.getPollingPeriod());
            timeoutFuture = scheduler.scheduleWithFixedDelay(this, updateInterval * 2, updateInterval, TimeUnit.MILLISECONDS);
            listener.addPayloadHandler(this);
            listener.open();
            logger.debug("Listening to meter updates and publishing its measurements every {}ms for '{}'", updateInterval, getThing().getUID());
    ...
    public void run() {
        ...
        if (currentTimeMillis - lastUpdate > updateInterval * 2) {
            ...
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.GONE, "No update received within " + missedWindow + " seconds");
        }
    }
